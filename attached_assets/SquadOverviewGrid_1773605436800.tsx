...
const DraggableSquadRow = ({ squad, index, moveSquadRow, getSwimmersInSquadByAge, visibleAgeGroups }: DraggableSquadRowProps) => {
  ...
  return (
    <div ref={(node) => preview(drop(node))} className={cn('', isDragging && 'opacity-50')}>
      <div className="flex border-b hover:bg-accent/20 transition-colors">
        {/* Squad name column with drag handle */}
        <div className="sticky left-0 z-10 bg-card border-r flex items-center" style={{ minWidth: '200px', width: '200px' }}>
          <div
            ref={drag}
          ...
          <div className="flex-1 px-3 py-6 font-medium">
            {squad.name}
          </div>
        </div>

        {/* Age group columns */}
        {AGE_GROUPS.filter(ag => visibleAgeGroups.has(ag)).map((ageGroup) => {
...
export function SquadOverviewGrid({ swimmers, squads }: SquadOverviewGridProps) {
  ...
  // Calculate age as at end of 2026
  ...
  const getSwimmersInSquadByAge = (squadId: string, ageGroup: AgeGroup): Swimmer[] => {
    return swimmers.filter(
      swimmer => swimmer.squadId === squadId && getAgeGroup(swimmer.dateOfBirth) === ageGroup
    );
  };

  // Get count for a specific age group (for header)
  ...
  const orderedSquads = squadOrder
    .map(id => squads.find(s => s.id === id))
    .filter((s): s is Squad => s !== undefined && visibleSquads.has(s.id));

  const visibleAgeGroupsOrdered = ageGroupOrder.filter(ag => visibleAgeGroups.has(ag));
    ...
    <DndProvider backend={HTML5Backend}>
      <div className="space-y-4 h-full flex flex-col">
          ...
          <div className="overflow-auto flex-1">
            <div className="inline-block min-w-full">
              ...
              <div className="flex border-b bg-muted/30 sticky top-0 z-20">
                ...
                {visibleAgeGroupsOrdered.map((ageGroup, index) => (
        ...
        <div className="text-sm text-muted-foreground flex-shrink-0">
          ...
          <p className="mt-1">
            Age groups are calculated as of December 31, 2026.
          </p>
        </div>
      </div>
    </DndProvider>
